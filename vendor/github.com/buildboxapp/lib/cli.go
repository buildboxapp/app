...
func (c *Lib) Ls() (result []map[string]string) {
	...
	files, err := ioutil.ReadDir(CurrentDir() + "/ini")
	if err != nil {
		log.Fatal(err)
	}
...
// format - тип вывода (terminal - пишем в терминал списко процессов, pid - список пидов, full - полный слайс значений как для терминала, но в структуре)
func (c *Lib) Ps(format string) (pids []string, services map[string][]string, err error) {
